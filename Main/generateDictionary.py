...
def generateDictionary_Exp(N_models, model_type, layers, input_size, hidden_size, image_size, loss_fn, dtype, hyperparameter):
	...
	modelBlock["Meta"] = {"Model_Type": model_type, "Epochs_Trained": 0, 
		"Type": dtype, "N": image_size, "Distribution": distribution}

	for i in layers:
		modelBlock[i] = {}
		# Set up model dictionaries with meta entires that stores key properties of the model
		modelBlock[i]["Meta"] = {"Layers": i, "Loss_Function": loss_fn}

		lr = hyperparameter[model_type][i]["Learning"]
		batch_size = hyperparameter[model_type][i]["Batch"]
		weight_decay = hyperparameter[model_type][i]["Weight_Decay"]
		...
		for j in range(N_models):
			...
			modelInit_Experiment(modelBlock, model_type, i, j, input_size, hidden_size, i, image_size)
			modelBlock[i][j]["Model"].type(dtype)
			modelBlock[i][j]["Learning"] = lr
			modelBlock[i][j]["Batch"] = batch_size
			modelBlock[i][j]["Optimizer"] = optim.Adam(modelBlock[i][j]["Model"].parameters(), lr = lr, weight_decay = weight_decay)
			modelBlock[i][j]["Loss"] = 100.0
			modelBlock[i][j]["Accuracy"] = 1.0
...
def modelInit_Experiment(modelBlock, model_type, key_layer, key_model, input_size, hidden_size, layers, image_size):
	logger = logging.getLogger(__name__)
	if (model_type == "DeepNet"):
		modelBlock[key_layer][key_model]["Model"] = NF.DeepNet(input_size, hidden_size, input_size, layers)
	elif (model_type == "DeepNetInput"):
		modelBlock[key_layer][key_model]["Model"] = NF.DeepNetInput(input_size, hidden_size, input_size, layers)
	elif (model_type == "Recurrent"):
		modelBlock[key_layer][key_model]["Model"] = NF.Recurrent(input_size, hidden_size, input_size, layers)
	elif (model_type == "RecurrentScaled"):
		modelBlock[key_layer][key_model]["Model"] = NF.RecurrentScaled(input_size, hidden_size, input_size, layers)
	elif (model_type == "RecurrentMasked5"):
		# This produces a model with a 5 x 5 gird around the pixel
		# The 2 indicated that we want the grid to start two pixels in every direction of a given pixel
		# All masked layers require an additional call to maskWeights to zero out unwanted weights
		modelBlock[key_layer][key_model]["Model"] = NF.RecurrentScaledMasked(input_size, hidden_size, input_size, layers, image_size, 2)
	elif (model_type == "RecurrentGrid"):
		# All masked layers require an additional call to maskWeights to zero out unwanted weights
		modelBlock[key_layer][key_model]["Model"] = NF.RecurrentScaledGrid(input_size, hidden_size, input_size, layers, image_size)
	elif (model_type == "RecurrentMultiplicative"):
		modelBlock[key_layer][key_model]["Model"] = NF.RecurrentScaledMultiplicative(input_size, hidden_size, input_size, layers)
	else:
		logger.warning('Model type not recognized')


def convertStateDict_Hyp(modelBlock):
...
def convertStateDict_Exp(modelBlock):
		...
		if (key_layer != "Meta"):
			for key, val in modelBlock[key_layer].items():
				if (key != "Meta"):
					model = modelBlock[key]["Model"].state_dict()
					optimizer = modelBlock[key]["Optimizer"].state_dict()
					modelBlock_State[key_layer][key]["Model"] = model
					modelBlock_State[key_layer][key]["Optimizer"] = optimizer
