...
def FindEdgePixels(x, dtype):
	...
	x_edge = np.zeros((image_size, image_size))

	for i in range(0, l):
		if ((row[i] ==0) or (col[i] == 0) or (row[i] == image_size-1) or (col[i]) == image_size-1):
			x_edge[row[i], col[i]] = 1


	x_edge[x_edge==0] = -1
	x_edge = np.reshape(x_edge, (1, num_pixels))
	x_edge = torch.from_numpy(x_edge).type(dtype)
